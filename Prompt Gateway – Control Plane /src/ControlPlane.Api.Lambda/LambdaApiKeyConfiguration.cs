...
namespace ControlPlane.Api.Lambda;

internal static class LambdaApiKeyConfiguration
{
    ...
    private static async Task<string> ResolveAsync(string valueFrom, CancellationToken cancellationToken)
    {
        if (IsSsmReference(valueFrom))
        {
            ...
            {
                throw new InvalidOperationException("Resolved control-plane API key parameter was empty.");
            }
        ...
        throw new InvalidOperationException("Resolved control-plane API key secret was empty.");
    }
