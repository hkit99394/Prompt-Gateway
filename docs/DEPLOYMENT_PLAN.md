...
**Automation:** Run `./scripts/first-deploy-phase3.sh --processing-mode ecs|lambda` to execute Phase 3 for the selected runtime. `ecs` keeps queue processing on ECS and turns Lambda processing off. `lambda` packages and enables Lambda processing while scaling ECS queue workers down. Requires Phase 1 and Phase 2 complete, Docker and `jq` installed. Optional env vars: `ENV` (dev | staging | prod), `IMAGE_TAG` (default: current git SHA, or UTC timestamp fallback), `AWS_REGION` (default: us-east-1), `HTTP_EDGE_MODE` (default: `lambda`), and `DEPLOY_ECS_API_SERVICE` (`auto`, `true`, or `false`). Phase 3 now always builds both container images and refreshes both ECS service task definitions so rollback remains deployable even when Lambda is the active runtime. In the default `lambda` + `HTTP_EDGE_MODE=lambda` path, the selected Lambda HTTP edge is expected to scale the ECS `control-plane-api` service to zero, but the ECS services are still repointed at the latest non-placeholder task definitions for rollback. Use `DEPLOY_ECS_API_SERVICE=true` or `HTTP_EDGE_MODE=ecs` when you explicitly want to refresh the ECS API service too. Use `--build-only` to only build and push images without deploying; use `--skip-verify` to skip the final runtime verification. Terraform ECS task definitions now take explicit `api_image_tag` and `worker_image_tag` inputs and default to a `bootstrap` placeholder until a real build tag is promoted, and runtime verification now fails if rollback task definitions still point at those placeholders.
...
**Automation:** Run `./scripts/first-deploy-phase4.sh` to execute T-5.4.1 – T-5.4.5 after either ECS mode or Lambda mode is deployed. Uses `scripts/smoke-test.sh`; resolves BASE_URL from API Gateway by default, or from the environment’s ALB when `HTTP_EDGE_MODE=ecs`, unless `BASE_URL` / `HEALTH_CHECK_BASE_URL` is set explicitly. The script uploads a smoke prompt fixture to S3 by default for the prepared prompt path, then verifies both job submission modes: a prompt reference job and an inline `promptText` job. It resolves the API key from SSM (dev) or Secrets Manager (staging/prod). Optional env vars: `ENV`, `HTTP_EDGE_MODE`, `VERIFY_ECS_HTTP_STATE`, `BASE_URL`, `API_KEY`, `AWS_REGION`, `SMOKE_INPUT_REF`, `SMOKE_PROMPT_BUCKET`, `SMOKE_PROMPT_TEXT`, `SMOKE_INLINE_PROMPT_TEXT`, `SMOKE_SKIP_PROMPT_UPLOAD=true`. Use `--insecure` for HTTPS with self-signed or ALB hostname when not using a custom domain. By default, the script now verifies that the ECS `control-plane-api` service matches the selected HTTP edge mode: off for `HTTP_EDGE_MODE=lambda`, on for `HTTP_EDGE_MODE=ecs`. Requires Phase 1–3 complete and `jq` installed. On failure, the smoke test now prints the final `GET /jobs/{jobId}` payload and `GET /jobs/{jobId}/events` timeline to speed up diagnosis.
